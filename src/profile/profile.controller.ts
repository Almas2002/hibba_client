...
export class ProfileController {
    ...
    @ApiQuery({name: 'hobby', type: 'sting', required: false, example: '1,2,3'})
    ...
    @ApiQuery({name: 'kids', type: 'boolean', required: false})
    @ApiBearerAuth()
    @UseGuards(AuthGuard)
    @Get('get-profiles')
    ...
    @UseGuards(RoleGuards)
    @Put('worker')
    updateWorker(@UserDecorator('id')id: number, @Body()dto: UpdateWorkerDto) {
